...
//-----------------------------------------------------------------------
int mbicp::MbICPmatcher(PTpfp laserRef, PTpfp laserNew, const ArPose &sensorMotion, int &numIteration, ArPose &solution,matrix &cov)
{
...
#ifdef PM_TEST
	ofstream f2;
	f2.open("D:\\Matlab Project\\useful data\\SceneData\\convergence\\convergence.txt", ios::app);
	if( !f2.is_open() )
	{
		cout<<"open file failed!"<<endl;
		//continue;
	}
	f2<<estim_cp.getX() * 100<<"  "<<estim_cp.getY() * 100<<"  "<<estim_cp.getThRad() / M_PI * 180<<endl;
	f2.close();
#endif
